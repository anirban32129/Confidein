...
function hydratePaymentPage(){
  ...
  function txnIdsFilled(){
    return !!(upiInput && googleInput && upiInput.value.trim() && googleInput.value.trim());
  }
  ...
  function refreshContinueState(){
    ...
    if(continueHint){
      continueHint.textContent = ready
        ...
        : 'Paste both transaction IDs from your payment app above to continue.';
    }
  }
  ...
  if(continueBtn){
    continueBtn.addEventListener('click', function(e){
      e.preventDefault();
      if(!txnIdsFilled()){
        if(upiInput && !upiInput.value.trim()) upiInput.focus();
        else if(googleInput) googleInput.focus();
        return;
      }
      const upiTxnId = upiInput.value.trim();
      const googleTxnId = googleInput.value.trim();
